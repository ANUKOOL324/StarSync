...
import {
  assignProblemsToCompetingRoom,
  getCompetingRoomDifficulty,
  getCompetingRoomDuration,
  getCompetingRoomTopics,
  getRoomPurpose,
} from "./competingRoomService";
import { isContestEnded } from "./contestTimingService";
import {
...
export const getRoomByIdOrSlug = async (roomId: string, userId: string) => {
  ...
  // Nobody writes ENDED when the timer runs out, so report it here instead.
  if (isContestEnded(safeRoom)) {
