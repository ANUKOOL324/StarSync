    ...
    <>
        ...
        <div className="rounded-[20px] border border-white/16 bg-linear-to-b from-[#303033]/95 via-[#242426]/95 to-[#202022]/95 px-6 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.14),0_18px_55px_rgba(0,0,0,0.32)]">
          ...
          <div className="grid gap-5">
            {isAdmin ? (
              <form onSubmit={onRename} className="grid gap-3">
                <label className="grid gap-2 text-sm text-zinc-300">
                  Rename room
                  <Input name="name" defaultValue={room.name} placeholder="Room name" />
                </label>
                <Button type="submit" size="sm" className="h-10 w-fit cursor-pointer justify-self-end rounded-full border-2 border-white/10 bg-transparent px-5 text-sm font-semibold text-white shadow-none transition-all duration-300 hover:border-white/22 hover:bg-white/8 hover:text-white">
                  Save name
                </Button>
              </form>
            ) : (
            ...
            <div className="mt-2 flex justify-end gap-2">
              <Button type="button" variant="ghost" size="sm" onClick={onLeave} className="h-10 cursor-pointer rounded-full border-2 border-white/10 bg-transparent px-5 text-sm font-semibold text-white shadow-none transition-all duration-300 hover:border-white/22 hover:bg-white/8 hover:text-white">
                <LogOut size={14} aria-hidden="true" />
                Leave room
              </Button>
              {isAdmin ? (
                <Button type="button" variant="ghost" size="sm" onClick={onOpenDelete} className="h-10 cursor-pointer rounded-full border-2 border-red-500/15 bg-transparent px-5 text-sm font-semibold text-red-400 shadow-none transition-all duration-300 hover:border-red-500/30 hover:bg-red-950/10 hover:text-red-300">
      ...
      <Modal isOpen={isDeleteConfirmOpen} onClose={onCloseDelete} title="Delete room" size="lg" hideHeader className="relative overflow-visible rounded-[28px] border border-white/10 bg-black/50 p-5 shadow-[0_26px_80px_rgba(0,0,0,0.58)]">
        <div className="rounded-[20px] border border-white/16 bg-linear-to-b from-[#303033]/95 via-[#242426]/95 to-[#202022]/95 px-6 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.14),0_18px_55px_rgba(0,0,0,0.32)]">
          ...
          <div className="grid gap-5">
            <p className="text-center text-sm leading-6 text-zinc-300">Are you sure you want to delete this room?</p>
            <div className="grid gap-2 sm:grid-cols-2">
              <Button type="button" variant="ghost" onClick={onCancelDelete} className="h-10 cursor-pointer rounded-full border-2 border-white/10 bg-transparent px-5 text-sm font-semibold text-white shadow-none transition-all duration-300 hover:border-white/22 hover:bg-white/8 hover:text-white">Cancel</Button>
              <Button type="button" variant="ghost" onClick={onDelete} className="h-10 cursor-pointer rounded-full border-2 border-red-500/15 bg-transparent px-5 text-sm font-semibold text-red-400 shadow-none transition-all duration-300 hover:border-red-500/30 hover:bg-red-950/10 hover:text-red-300">
                <Trash2 size={14} aria-hidden="true" />
                Delete room
              </Button>
            </div>
